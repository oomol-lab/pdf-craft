...
发版由手动触发的 GitHub Actions 发版工作流执行。在运行该工作流之前，仓库必须先完成准备。该工作流是发版关口：它会从 `pyproject.toml` 读取版本号，检查仓库状态是否可发布，发布包到 PyPI，推送版本标签，并使用变更日志文件作为发布说明创建 GitHub Release。
...
- 使用 `uv version X.Y.Z` 同步更新 [pyproject.toml](../pyproject.toml) 中的 `project.version` 和 `uv.lock`。
