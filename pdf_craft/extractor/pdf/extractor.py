...
class PDFExtractor:
    ...
    def _extract_to_workspace(
        ...
        return self._load_workspace(analysing_path, metering)

    @staticmethod
    def _load_workspace(analysing_path: Path, metering):
    ...
    async def extract_with_metering(
        ...
        **kwargs: Any,
        ...
        if getattr(self._transform, "extract_package_async", None) is not None:
            if extraction_path.suffix.lower() != EXTRACTION_SUFFIX:
                raise ValueError(
            ...
            with _analysis_workspace(analysing_path) as workspace:
                ...
                exported = await extraction._export_async(extraction_path)
            return exported, metering
    ...
    async def _extract_to_workspace_async(
        ...
        **kwargs: Any,
        ...
        extract_package_async = getattr(
            self._transform, "extract_package_async", None
        )
        if extract_package_async is not None:
            defaults = {
                "analysing_path": analysing_path,
                "ocr_size": "gundam", "dpi": None,
                "max_page_image_file_size": None, "includes_cover": False,
                "includes_footnotes": False, "includes_furniture": True,
                "ignore_pdf_errors": False, "ignore_ocr_errors": False,
                "generate_plot": False, "toc_llm": None,
                "toc_assumed": False, "extract_book_metadata": False,
            ...
            return await IO_DOMAIN.run(
                self._load_workspace, analysing_path, metering
            )
