...
When no `sub_title` style is configured, its effective maximum expands to meet the actual body-relative lower bound, including one introduced by a `text` or `text:<level>` style. An explicitly configured `sub_title` or `sub_title:<level>` maximum remains a hard limit. If the resulting headline cannot fit its width at the required minimum size, it is anchored at the source box's left edge and continues naturally to the right rather than failing or being skipped.
