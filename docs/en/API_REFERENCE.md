...
`translate_extraction()` accepts a caller-supplied opaque `translation_id` or generates a short one. IDs, not languages, identify layers, so one target language may have several translations. It stores `target_language` in the layer metadata; when it cannot infer a language and none is supplied, it uses `und`. Duplicate IDs are rejected before translation. PCEX layers contain replacement text only, so this method rejects append submit modes. `with_furniture=True` stores translated page furniture inside the new layer and requires `ChapterXMLTransformer`. Asset text remains source content. Current renderers do not yet select these layers; `translate_pdf()` continues to use an internal materialized translation for its existing behavior.
