...
- `max_retries` 控制单个翻译或结构修复任务允许的最大总尝试次数，默认值为 5；因此默认
  最多包含 4 次重试。`FillFailedEvent.retried_count` 记录的也是当前总尝试次数。
- `max_group_tokens` 控制一次提交给翻译器的文本分组大小，默认值为 2600。增大它可以
  减少请求次数，但会增加单次请求的上下文和失败重试成本。
- LLM 配置中的 `retry_times` 控制底层请求遇到网络错误或空响应时的额外重试次数；默认值 5
  表示首次请求加最多 5 次重试（最多 6 次尝试）。它和 `max_retries` 处于不同层次，前者
  针对请求传输，后者针对翻译结构任务。
...
`on_progress` 接收一个从 `0.0` 到 `1.0` 的进度值。实现按固定权重推进：目录占 5%、元数据
占 5%，章节阶段占 90%。没有目录或元数据时，对应阶段不回调；没有章节时，章节的 90% 也
不会重新分配。因此某些 EPUB 的最终进度可能停在 `0.05`、`0.10`、`0.90` 或 `0.95`，这是当前实现
的限制，不应把回调值当作必然达到 `1.0` 的完成信号。
