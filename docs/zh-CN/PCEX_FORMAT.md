...
可变页码以结构化信息表示，而不是作为可复用 pattern position 的文本：folio `position` 带有 `folio_style`（`D`、`R`、`r`、`A` 或 `a`）和 `folio_offset`，也可带 `folio_prefix`、`folio_suffix`。关联到每页的 section 仍保留实际印刷出的页码，从而不会把抽样页的页码翻译一次后错误地写到所有共享该 position 的页面。
